...
import { CompanyStatusEnum } from '@/lib/api/schemas';
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = CompanyStatusEnum.parse(body?.to);
    ...
    try {
      ...
      await supabase.from('company_events').insert({ company_id: companyId, org_id: orgId, actor_membership_id: actorId, type: 'STATUS_CHANGE', payload: { from: company.status, to: parsed } });
    } catch {}
